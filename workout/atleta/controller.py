...
from workout.atleta.schemas import AtletaIn, AtletaOut, AtletaUpdate
...
async def post(db_session: DatabaseDependency, atleta_in: AtletaIn = Body(...)):
    ...
    categoria = (await db_session.execute(select(CategoriaModel)
                 .filter_by(nome=categoria_nome))).scalar().first()
    ...
    centro_treinamento = (await db_session.execute(select(CentroTreinamentoModel)
                 .filter_by(nome=centro_treinamento_nome))).scalar().first()

    if not centro_treinamento:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
             detail=f"o centro de treinamento {centro_treinamento_nome} não encontrada")
    ...
    try:
        ...
        atleta_model.centro_treinamento_model.categoria_id = centro_treinamento.pk_id
